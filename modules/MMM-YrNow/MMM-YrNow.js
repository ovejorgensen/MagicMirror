Module.register("MMM-YrNow", {
	...
	socketNotificationReceived: function (notification, payload) {
		if (notification === "YR_FORECAST_DATA") {
			if (payload.nowcast.points != null) {
				...
				if (this.config.showWeatherForecast) this.processForecast(payload.forecast);
			}
			this.updateDom(1000);
		}
	},
	...
	getNextPrecipStart: function () {
		return this.list.points.filter((item) => item.precipitation.intensity > 0 && Date.parse(item.time) >= new Date().valueOf())[0];
	},
